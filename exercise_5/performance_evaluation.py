...
from tools.sequence_utils import VOTSequence, read_results


def estimate_thresholds(scores, n):

    scores_vector = np.sort(
        np.concatenate([np.array(s_) for s_ in scores], axis=0), axis=0
    )
...
def evaluate_performance(dataset_path, results_dir, return_results=False):
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='Longe-term Tracking Performance Evaluation Script'
    )
